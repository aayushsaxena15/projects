        ...
        query parse( string input )
        {
            ...
            {    
                ...
                for(int i=0;i<words.size()-1;i++)
                {
                    ...
                    {
                        while(j<words.size() && words.at(j).compare("where")!=0 && words.at(j).find("orderby")!=0)
                            { q.tables.append( words.at(j) ); j++;}
			cout << words.at(j-1) << " fsdafas\n";
                    }
...
//    file1.queryType("select s_id,s_order,s_num from table2 where table2.s_num<230 and table2.s_num>205 or table2.s_idLIKE48 groupby s_id having table2.s_order=50 orderby table2.s_num(DESC) ");
    ...
    file1.queryType("select e_id,s_id from table1 join table2 on table1.e_id=table2.s_id");
